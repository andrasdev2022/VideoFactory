...
class VisualHoldTimingTests(unittest.TestCase):
    def setUp(self):
        ...
        self.spec = {'video': {'target_duration_sec': 30, 'min_duration_sec': 20, 'max_duration_sec': 45}}
        self.job = {'script': {'duration_normalization': {'status': 'remeasured', 'changed_scene_ids': [1, 2, 3, 4, 5, 6]}, 'scenes': []}}
        for i, duration in enumerate([5, 4.85, 4.8, 3.45, 2.95, 4.2], 1):
    ...
    def test_romance_recovery_preserves_voice_and_finishes_with_exhausted_budget(self):
        ...
        self.assertAlmostEqual(sum(s['timing']['render_duration_sec'] for s in self.job['script']['scenes']), 30)
    ...
    def test_limits_redistribute_hold(self):
        self.job['script']['scenes'][0]['timing']['max_video_duration_sec'] = 5.1
        self.assertTrue(extend_visual_holds(self.job, self.spec))
        self.assertEqual(self.job['script']['scenes'][0]['timing']['render_duration_sec'], 5.1)
        self.assertAlmostEqual(self.job['timing_summary']['total_render_duration_sec'], 30)
    ...
    def test_real_orchestrator_resumes_exhausted_job_without_workers(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertAlmostEqual(result['timing_summary']['total_render_duration_sec'], 30)
